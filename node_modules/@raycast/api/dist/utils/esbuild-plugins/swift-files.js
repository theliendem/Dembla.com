...
`;return this.stop(),this.#i.write(c),this}};function ei(e){return new Qo(e)}var B3=(0,P.blue)((0,P.dim)("internal only"));function bt(e,t,r){console.log(_f[e]+t),typeof r?.exit<"u"&&process.exit(r.exit)}async function Tf(e,t,r){if(!zF){bt("wait",e);try{let o=await t();o&&console.log(o),bt("success",e);return}catch(o){return bt("error",e),r?.printError!==!1&&console.log((0,P.red)(o.message)),o}}let n=ei({spinner:"simpleDots",prefixText:_f.wait+e}).start();try{let o=await t();n.stop(),bt("success",e),o&&console.log(o)}catch(o){return n.stop(),bt("error",e),r?.printError!==!1&&console.error(o.message),o}}var _f={wait:`\u{1F550}${(0,P.blue)("wait")}  - `,info:`${process.env.GITHUB_ACTIONS==="true"?"\u2139\uFE0F":"\u2139\uFE0F "}${(0,P.cyan)("info")}  - `,success:`\u2705${(0,P.green)("ready")}  - `,warn:`${process.env.GITHUB_ACTIONS==="true"?"\u26A0\uFE0F":"\u26A0\uFE0F "}${(0,P.yellow)("warn")}  - `,error:`\u{1F4A5}${(0,P.red)("error")}  - `,event:`\u26A1\uFE0F${(0,P.magenta)("event")}  - `,paymentPrompt:`\u{1F4B0}${(0,P.yellowBright)("plan")}  - `},zF=!0;function VF(e,t,r){let n=e?"release":"debug",o=/#exportFunction\((\w+)\)/g;return{name:"swift-files",setup(i){i.onResolve({filter:/^swift:/},s=>({path:M.default.join(s.resolveDir,s.path.replace(/^swift:/,"")),namespace:"swift",sideEffects:!1})),i.onLoad({filter:/.*/,namespace:"swift"},async s=>{let a=s.path,u=M.default.basename(a),l=M.default.join("compiled_raycast_swift",u),c=[],f="",D="",d="Sources",p=".raycast-swift-build",g=M.default.join(a,p);if(!b.existsSync(M.default.join(a,"Package.swift")))return{errors:[{text:`failed to import Swift package from "${a}". Make sure you reference the folder that includes the 'Package.swift' file`}]};if(await Tf("compiling swift package",async()=>{let _=await Rf("/bin/sh",["-c","swift package dump-package"],a,g),j;try{j=JSON.parse(_)}catch(B){throw new Error(`failed to parse the swift package: ${B}`)}if(f=j.name,!f)throw new Error("failed to read the schema name from swift package");for(let B of j.targets||[])if(B.type==="executable"){if(D)throw new Error("failed to parse the swift package: expected one executable target but found more than one");D=B.name,B.path&&(d=B.path)}if(!D)throw new Error("failed to parse the swift package: expected one executable target but found none");let I=`xcodebuild build -skipMacroValidation -skipPackagePluginValidation -configuration ${n} -scheme ${f} -destination "generic/platform=macOS,name=Any Mac" -derivedDataPath ${p}`;await Rf("/bin/sh",["-c",I],a,g);let L=M.default.join(g,"Build/Products",n,D),Fe;try{if(Fe=b.statSync(L),(Fe.mode&73)===0)throw new Error("not executable")}catch{throw new Error(`failed to compile swift: Could not find the compiled asset.

Try to delete the ${M.default.join(g,"apple/Products",n)} folder and try again`)}if((!b.existsSync(M.default.join("assets",l))||!KF(L,M.default.join("assets",l)))&&b.cpSync(L,M.default.join("assets",l),{recursive:!0,force:!0}),t){let B=M.default.join(a,d),qr=process.cwd();try{B=M.default.relative(qr,B)}catch{}if(!b.existsSync(B))throw new Error(`failed to compile swift: Could not find Swift sources directory ${B}`);t(B)}},{printError:!0}))return{errors:[{text:`failed to import Swift package from "${a}". Make sure you reference the folder that includes the 'Package.swift' file`}]};let y="",R=M.default.join(g,`SourcePackages/plugins/${u.toLowerCase()}.output`,D,"RaycastTypeScriptPlugin");if(b.existsSync(R)){let _=M.default.join(R,"raycast.d.ts");try{let I=b.readFileSync(_,"utf-8");r(qF(u,I))}catch(I){throw new Error(`failed to compile swift: Could not load the content of ${_}
 ${I}`)}let j=M.default.join(R,"raycast.js");try{let I=b.readFileSync(j,"utf-8");return{contents:Of(l,I),loader:"js"}}catch(I){return{errors:[{text:`failed to compile swift: Could not load the content of ${_}
 ${I}`}]}}}function N(_){let j=b.readdirSync(_);for(let I of j){let L=M.default.join(_,I);if(b.statSync(L).isDirectory())N(L);else if(I.endsWith(".swift")){let B=b.readFileSync(L,"utf-8").matchAll(o);for(let qr of B)c.push(qr[1])}}}N(M.default.join(a,"Sources"));let k="";for(let _ of c)k+=`export const ${_} = proxy.${_};
`;return y=Of(l,k),r(YF(M.default.basename(a),c)),{contents:y,loader:"js"}})}}}function YF(e,t){let r="";for(let n of t)r+=`    export const ${n}: <T = unknown, U = any>(input?: U) => Promise<T>;
